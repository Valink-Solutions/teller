...
#[tauri::command]
pub fn update_category_name(old_name: &str, new_name: &str) -> Result<(), String> {
    let config_dir = get_config_folder();
    let mut saves_config = match get_saves_config(&config_dir) {
        Ok(s) => s,
        Err(e) => {
            error!("Could not get saves config: {:?}", e);
            return Err(format!("Could not get saves config: {:?}", e));
    ...
    if let Some(category) = saves_config.categories.remove(old_name) {
        saves_config
            .categories
            .insert(new_name.to_string(), category);
        match fs::write(
            &config_dir.join("local-directories.json"),
            serde_json::to_string(&saves_config).unwrap(),
        ) {
            Ok(_) => (),
            Err(e) => {
...
pub fn update_path_name(
    ...
    if let Some(category) = saves_config.categories.get_mut(category_name) {
        if let Some(path) = category.paths.remove(old_path_name) {
            category.paths.insert(new_path_name.to_string(), path);
            match fs::write(
                &config_dir.join("local-directories.json"),
                serde_json::to_string(&saves_config).unwrap(),
            ) {
